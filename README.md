...
ID 不采用全项目统一自增。默认按目标 Sheet 局部唯一，只有 B1 明确共享时才扩大到模块或父对象命名空间；不同无关 Sheet 可以复用相同数字。

- 优先级：策划明确值 → B1 构成规则 → 已占用值/保留间隔 → 父 ID 和外键关系 → 待确认候选
- 保留 B1 定义的模块段、类型段、位宽、父 ID 前缀和插入步长，不压缩或重排旧 ID
- `max+1` 只适用于 B1 明确的连续自增表，不能跨表通用
- 测试 ID 也必须符合 B1 规则；没有正式测试段时标记为 `candidate`
- `99`、`9000`、`999999`、`9999999` 等旧表保留值不能随意用于新增数据
- 每个新增或修改 ID 都要写入 `generation-manifest.json` 的 `id_allocations` 台账
